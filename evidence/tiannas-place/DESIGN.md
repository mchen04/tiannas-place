...
Resolution on the resumed supervisor turn: the supervisor verified the Vercel project’s stable numeric GitHub repository binding using existing credentials on mbp-main, without transferring credentials or changing configuration. The authorized GitHub name-only rename then completed, preserving identity, history, access, redirects and the live origin. The paragraph above records the original preflight state. [Completed rename proof](RENAME.md).

## Review refinements · F1–F6

The brand remains the Home heading, with the original time-of-day/progress greeting restored visibly in the date block below it. It had temporarily existed only in the heading's accessible name; the visible greeting now supplies that information once to both sighted and screen-reader users.

Dashboard chevrons are intentionally omitted. Their width competed with readable labels and the independent action target at enlarged text. The activity name/art/status button opens details, while the neighboring checkbox or labeled action logs/completes; the two controls remain separately named, focusable and reachable in the original tap counts. Pressed backgrounds now round all four corners, including when the body wraps above its action, without clipping the focus ring.

The walking art is unchanged. Home and Walk now show at least the lower 220 SVG units and have a 220px minimum scene height, so the cloud is fully inside the viewport even at the maximum shell width. The cats keep their 360:160 ratio as the stage width changes, avoiding both crop and white gutters. Mat completion continues to use copy and the stage tag; its cat stretches during active feedback. This corrects the earlier comment rather than adding a new completion animation.

The token catalogue is retained, including currently unused entries; only the identified inert rules/options are removed. Historical scan snapshots remain unchanged, with new scan output stored under this card. See [review follow-up proof](review-polish/PROOF.md) for source-specific evidence and command side effects.
